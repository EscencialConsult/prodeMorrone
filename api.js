...
function routeGet_(action, params) {
  ...
  switch (action) {
    ...
    case 'predicciones.tabla_global':return prediccionesTablaGlobal_(params);
...
// ── Ranking Global (suma de todas las apuestas, solo lectura) ─────────
function prediccionesTablaGlobal_(params) {
  const user = requireSession_(params.session_token);
  const limit = Math.min(parseInt(params.limit) || 50, 200);
  ...
  let ranking = null;
  try {
    const cached = cache.get('ranking_global');
    if (cached) ranking = JSON.parse(cached);
  } catch (e) {}

  if (!ranking) {
    ranking = construirRankingGlobal_();
    try {
      const serialized = JSON.stringify(ranking);
      if (serialized.length < 100000) cache.put('ranking_global', serialized, CACHE_TTL_RANKING);
    } catch (e) {}
  }
  ...
  return {
    ok: true, apuesta_titulo: 'Ranking Global',
    total: ranking.length, limit, tabla: top,
    mi_posicion: miPosicion, esta_en_top: estaEnTop
  };
}

function construirRankingGlobal_() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEETS.PREDICCIONES);
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return [];
  const headers = getHeaders_(SHEETS.PREDICCIONES);
  ...
  const usuariosMap = new Map(getSheet_(SHEETS.USUARIOS).map(u => [u.id, u]));
  const apuestasMap = new Map(getSheet_(SHEETS.APUESTAS).map(a => [a.id, a]));
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!tabla[uid]) {
      const u = usuariosMap.get(uid);
      tabla[uid] = { user_id: uid, nombre: u ? u.nombre : 'Desconocido',
        puntos_totales: 0, aciertos_exactos: 0, aciertos_diferencia: 0,
        aciertos_resultado: 0, aciertos_clasificado: 0, predicciones: 0 };
    }
    const puntos = parseInt(row[colPuntos]) || 0;
    ...
    if (apuesta) {
      const e = parseInt(apuesta.puntos_exacto)||5, d = parseInt(apuesta.puntos_diferencia)||3,
            r = parseInt(apuesta.puntos_resultado)||1, c = parseInt(apuesta.puntos_clasificado)||1;
      if (puntos === e) tabla[uid].aciertos_exactos++;
      else if (puntos === d) tabla[uid].aciertos_diferencia++;
      else if (puntos === c && puntos !== r) tabla[uid].aciertos_clasificado++;
      else if (puntos === r) tabla[uid].aciertos_resultado++;
    }
  }
  return Object.values(tabla).sort((a, b) => b.puntos_totales - a.puntos_totales);
}
