/**
 * RankingPageUser.jsx â€” Diseño premium v6 (USER VERSION)
 ...
 *
 * v6.1 â€” Dimensiones alineadas con FixturePage (maxWidth:1400, padding:'2rem 1.5rem 3rem')
...
export default function RankingPageUser() {
  ...
  return (
    <AppShell>
      ...
      {/* âš¡ Dimensiones alineadas con FixturePage */}
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'2rem 1.5rem 3rem' }}>

        {/* Título página â€” RANKING navy + (subtítulo gris debajo) */}
        <div className="rk-in" style={{ marginBottom:'1.5rem' }}>
          ...
          <p style={{ fontSize:'.84rem', color:'#6f7377', margin:0 }}>
            {modo==='global' ? 'Clasificación general · suma de todas las apuestas' : (sel ? sel.titulo : 'Seleccioná una apuesta para ver el ranking')}
          </p>
        </div>

        {/* Shell principal â€” altura ajustada al nuevo padding */}
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   MI POSICIÃ“N (sticky) - SIN EXPANDIR
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   OTROS PARTICIPANTES â€” Con toggle
â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
        ...
        <div style={{display:'flex',flexDirection:'column',gap:6,paddingTop:12}}>
          {otros.map((u, idx) => (
            <div key={u.user_id} style={{
              ...
              gap:10,
              padding:'9px 12px',
              background: u.user_id === user?.id ? 'rgba(184,160,106,.1)' : '#fff',
              border: u.user_id === user?.id ? '1.5px solid #b8a06a' : '1px solid #ece9e2',
              boxShadow: u.user_id === user?.id ? '0 0 0 1px rgba(184,160,106,.3), 0 2px 8px rgba(184,160,106,.12)' : 'none',
              borderRadius:10,
              alignItems:'center',
              transition:'all .15s',
            }}
              ...
              <div style={{minWidth:0}}>
                <p style={{fontWeight: u.user_id === user?.id ? 700 : 500,fontSize:11,color: u.user_id === user?.id ? '#b8a06a' : '#202124',margin:'0 0 1px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                  {u.nombre}
                  {u.user_id === user?.id && (
                    <span style={{
                      fontSize:8,
