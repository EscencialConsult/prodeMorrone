const ITEMS = [
  {
    bg: 'rgba(111,115,119,.12)', stroke: '#6f7377',
  ...
  {
    bg: 'rgba(184,160,106,.16)', stroke: '#8f7a45',
...
export default function HomeStrip() {
  return (
    <>
      ...
      {/* 4-feature strip */}
      <section style={{ background: '#f6f5f2', padding: '3.5rem 0' }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {ITEMS.map(({ bg, stroke, title, desc, icon }) => (
              <div key={title} className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-body font-semibold text-base mb-1" style={{ color: '#202124' }}>{title}</h3>
                  <p className="font-body text-sm leading-snug" style={{ color: '#6f7377' }}>{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Marquee */}
      <div className="marquee-wrap-el" style={{ background: '#dedbd4', borderTop: '1px solid rgba(184,160,106,.3)', borderBottom: '1px solid rgba(184,160,106,.3)', padding: '.9rem 0' }}>
        <div className="marquee-track-el font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#202124' }}>
          {[...MARQUEE, ...MARQUEE].map((item, i) => (
            <span key={i} className="flex items-center gap-2.5 whitespace-nowrap">
              <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: '#b8a06a' }} />
              {item}
            </span>
          ))}
        </div>
      ...
      {/* ── WAVE: strip (#f6f5f2) → how-it-works (#f6f5f2 same, wave goes to #202124 later)
