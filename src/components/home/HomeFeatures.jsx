...
export default function HomeFeatures() {
  return (
    ...
    <section id="funcionalidades" className="home-section relative" style={{ background: '#202124', paddingTop: '5.5rem', paddingBottom: '6rem' }}>
      <div className="home-section-inner" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-end mb-14">
          <div>
            ...
            <h2 className="home-section-heading font-display text-white" style={{ fontSize: 'clamp(2.4rem,6vw,4rem)', lineHeight: 1, letterSpacing: '.01em' }}>
              UNA PLATAFORMA<br /><span style={{ color: '#b8a06a' }}>COMPLETA</span>
            </h2>
          </div>
          <p className="font-body text-base leading-relaxed" style={{ color: '#a9adb1' }}>
            Todo lo que necesitás para participar, seguir el Mundial y competir con tu equipo está en un solo lugar.
          </p>
        </div>

        <div className="home-card-grid grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURES.map(({ title, desc, icon }) => (
            <div key={title} className="home-feat-card home-card rounded-xl p-5 transition-all duration-300"
              style={{ background: 'linear-gradient(155deg,rgba(111,115,119,.22),rgba(111,115,119,.05))', border: '1px solid rgba(184,160,106,.18)' }}
            >
              ...
              <p className="font-body text-xs leading-relaxed" style={{ color: '#a9adb1' }}>{desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 text-center">
          <Link to="/register"
            className="home-section-cta home-feat-cta inline-flex items-center gap-2 font-body font-bold text-base px-8 py-4 rounded-full transition-all"
            style={{ background: '#b8a06a', color: '#111214', boxShadow: '0 8px 24px rgba(184,160,106,.25)', textDecoration: 'none' }}>
      ...
      {/* WAVE: features (#202124) → FAQ (#f6f5f2)
          rect = navy (esta sección), path fill = cream (siguiente sección)
          ESTE ERA EL PROBLEMA: sin rect, el área transparente mostraba #111214 del wrapper */}
      <svg className="absolute bottom-0 left-0 w-full" style={{ display: 'block', height: 80, marginBottom: -2 }}
        ...
        <rect width="1440" height="80" fill="#202124" />
        <path d="M0,30 C320,80 640,0 960,38 C1120,55 1320,22 1440,32 L1440,80 L0,80 Z" fill="#f6f5f2" />
      </svg>
    </section>
  )
}
