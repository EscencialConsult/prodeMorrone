...
export default function Navbar() {
  ...
  const linkStyle = isActive => isActive
    ? { background: 'rgba(184,160,106,.15)', color: '#b8a06a', border: '1px solid rgba(184,160,106,.3)' }
  ...
  return (
    <nav
      ...
      style={{
        ...
        borderBottom: '1px solid rgba(184,160,106,.12)',
    ...
    >
      <div className="max-w-6xl mx-auto px-4 md:px-6 h-16 flex items-center gap-4">
        ...
        <Link to="/dashboard" className="flex items-center gap-3 flex-shrink-0 min-w-0">
          ...
          <div className="w-px h-5 hidden sm:block" style={{ background: 'rgba(184,160,106,.25)' }} />
          <MorroneLogo size={28} variant="light" markOnly style={{ filter: 'drop-shadow(0 2px 8px rgba(0,0,0,.5))' }} />
        </Link>
          ...
          {isAdmin && ADMIN_LINKS.map(link => (
            <NavLink key={link.to} to={link.to}
              ...
              style={({ isActive }) => isActive
                ? { background: 'rgba(184,160,106,.2)', color: '#b8a06a', border: '1px solid rgba(184,160,106,.4)', fontWeight: 700 }
                : { color: 'rgba(184,160,106,.6)', border: '1px solid transparent' }
              }
            >
              {link.label}
            </NavLink>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <div className="w-7 h-7 rounded-full flex items-center justify-center font-display text-xs"
                style={{ background: 'linear-gradient(135deg,#b8a06a,#8f7a45)', color: '#111214', boxShadow: '0 2px 8px rgba(184,160,106,.3)' }}>
