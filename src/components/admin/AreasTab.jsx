...
function BtnGold({ children, onClick, disabled, type = 'button' }) {
  return (
    <button
      ...
      style={{ background: '#202124', color: '#b8a06a', boxShadow: '0 4px 14px rgba(32,33,36,.2)' }}
      onMouseEnter={e => { if (!disabled) { e.currentTarget.style.background = '#3b3d40'; e.currentTarget.style.transform = 'translateY(-1px)' } }}
      onMouseLeave={e => { if (!disabled) { e.currentTarget.style.background = '#202124'; e.currentTarget.style.transform = '' } }}
    >
      {children}
    </button>
  )
...
function BtnOutline({ children, onClick, disabled, danger = false }) {
  const border  = danger ? 'rgba(224,50,82,.35)' : 'rgba(32,33,36,.2)'
  const color   = danger ? '#e03252'             : '#6f7377'
  const hoverBg = danger ? 'rgba(224,50,82,.06)' : 'rgba(32,33,36,.04)'
  return (
    <button
...
//             </svg>
//           </div>
//           <h2 className="font-display text-2xl" style={{ color: '#202124', letterSpacing: '.02em' }}>NUEVA ÁREA</h2>
//         </div>

//         <form onSubmit={handleCreateArea} className="flex flex-col gap-4">
//           <div>
//             <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>Nombre *</p>
...
//           </div>
//           <div>
//             <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>Descripción (opcional)</p>
...
//             {savingArea
//               ? <><span className="w-4 h-4 rounded-full border-2 border-t-transparent inline-block animate-spin" style={{ borderColor: '#b8a06a' }} /> Creando...</>
//               : 'Crear área'
//             }
//           </BtnGold>
//         </form>
...
//             <h2 className="font-display text-2xl" style={{ color: '#202124', letterSpacing: '.02em' }}>ÁREAS</h2>
//             <span className="font-display text-xl" style={{ color: '#b8a06a' }}>({areasAll.length})</span>
//           </div>
//           <BtnOutline onClick={loadAreasAll} disabled={loadingAreas}>
//             {loadingAreas
//               ? <span className="w-3 h-3 rounded-full border-2 border-t-transparent inline-block animate-spin" style={{ borderColor: '#6f7377' }} />
...
//             <span className="inline-block w-8 h-8 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: '#b8a06a' }} />
//             <p className="font-body text-sm mt-3" style={{ color: '#6f7377' }}>Cargando áreas...</p>
//           </div>
//         )}

//         {!loadingAreas && areasAll.length === 0 && (
//           <div className="rounded-2xl p-10 text-center" style={{ background: '#fff', border: '1px dashed #d8d5ce' }}>
...
//               </svg>
//             </div>
//             <p className="font-body text-sm font-semibold" style={{ color: '#6f7377' }}>Todavía no hay áreas creadas.</p>
//             <p className="font-body text-xs mt-1" style={{ color: '#a9adb1' }}>Creá la primera desde el formulario.</p>
//           </div>
//         )}
...
//                   style={{
//                     background: '#fff',
//                     border: `1px solid ${isActive ? '#dedbd4' : '#d8d5ce'}`,
...
//                           <p className="font-display text-lg truncate" style={{ color: '#202124', letterSpacing: '.01em' }}>{area.nombre}</p>
...
//                               color:      isActive ? '#1b8a5a'              : '#a9adb1',
//                               border:     `1px solid ${isActive ? 'rgba(27,138,90,.2)' : 'rgba(95,110,138,.15)'}`,
//                             }}>
//                             {isActive ? 'Activa' : 'Inactiva'}
//                           </span>
//                         </div>
//                         {area.descripcion && <p className="text-xs font-body mt-1" style={{ color: '#6f7377' }}>{area.descripcion}</p>}
//                       </div>
