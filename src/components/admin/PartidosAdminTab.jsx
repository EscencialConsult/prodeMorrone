...
const ESTADO_COLORS = {
  programado:  { color: '#6f7377', bg: 'rgba(255,255,255,.06)', border: 'rgba(255,255,255,.12)', label: 'Programado' },
  en_vivo:     { color: '#ff4d6d', bg: 'rgba(255,77,109,.12)',  border: 'rgba(255,77,109,.35)',  label: 'EN VIVO' },
  finalizado:  { color: '#b8a06a', bg: 'rgba(184,160,106,.1)',   border: 'rgba(184,160,106,.3)',   label: 'Finalizado' },
  cancelado:   { color: '#a9adb1', bg: 'rgba(255,255,255,.04)', border: 'rgba(255,255,255,.1)',  label: 'Cancelado' },
}
...
function EditScoreModal({ match, onClose, onSave }) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(32,33,36,.7)', backdropFilter: 'blur(8px)' }}
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center transition-all"
            style={{ background: '#f6f5f2', border: '1px solid #d8d5ce', color: '#6f7377' }}
        ...
        {/* Equipos + fase */}
        <p className="font-body text-sm text-center mb-1" style={{ color: '#6f7377' }}>
          <span className="text-white font-semibold">{match.equipo_local}</span>
          <span style={{ color: '#a9adb1' }}> vs </span>
          <span className="text-white font-semibold">{match.equipo_visitante}</span>
        </p>
        {match.fase && (
          <p className="font-body text-[10px] text-center mb-4 uppercase tracking-widest" style={{ color: elim ? '#b8a06a' : '#a9adb1' }}>
        ...
        {/* Score inputs (90') */}
        <p className="font-body text-[10px] uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>
          Marcador {elim ? '(90 minutos)' : ''}
        </p>
        <div className="flex items-center gap-4 mb-5">
          <div className="flex-1 text-center">
            <p className="font-body text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>{match.equipo_local}</p>
            <input
              ...
              style={{ background: '#f6f5f2', border: '1px solid #d8d5ce', color: '#b8a06a' }}
              onFocus={e => { e.target.style.borderColor = 'rgba(184,160,106,.5)'; e.target.style.boxShadow = '0 0 0 3px rgba(184,160,106,.1)' }}
              onBlur={e  => { e.target.style.borderColor = 'rgba(255,255,255,.12)'; e.target.style.boxShadow = 'none' }}
            />
          </div>
          <span className="font-display text-2xl" style={{ color: '#a9adb1' }}>:</span>
          <div className="flex-1 text-center">
            <p className="font-body text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>{match.equipo_visitante}</p>
            <input
        ...
        {/* Estado */}
        <div className="mb-5">
          <p className="font-body text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>Estado del partido</p>
        ...
        {debeMostrarPenales && (
          <div className="mb-5 p-3 rounded-xl" style={{ background: 'rgba(184,160,106,.06)', border: '1px solid rgba(184,160,106,.25)' }}>
            <div className="flex items-center gap-2 mb-2">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#b8a06a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/>
              </svg>
              <p className="font-body text-xs uppercase tracking-widest font-bold" style={{ color: '#b8a06a' }}>
                Definición por penales
              </p>
            </div>
            <p className="font-body text-[11px] mb-3" style={{ color: '#6f7377' }}>
              El partido terminó empatado en los 90'. Cargá los penales para que el sistema sepa quién clasifica.
            </p>
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <p className="font-body text-[10px] uppercase tracking-widest mb-1" style={{ color: '#6f7377' }}>{match.equipo_local}</p>
                <input
                  ...
                  style={{ background: '#fff', border: '1px solid #d8d5ce', color: '#202124' }}
                />
              </div>
              <span className="font-display text-lg" style={{ color: '#a9adb1' }}>:</span>
              <div className="flex-1">
                <p className="font-body text-[10px] uppercase tracking-widest mb-1" style={{ color: '#6f7377' }}>{match.equipo_visitante}</p>
                <input
        ...
        <div className="flex gap-2">
          <button onClick={onClose}
            ...
            style={{ background: 'transparent', border: '1px solid #d8d5ce', color: '#6f7377' }}
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            style={{ background: 'linear-gradient(135deg,#b8a06a,#8f7a45)', color: '#111214', boxShadow: '0 4px 16px rgba(184,160,106,.3)' }}
            onMouseEnter={e => { if (!saving) e.currentTarget.style.boxShadow = '0 6px 24px rgba(184,160,106,.5)' }}
            onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 16px rgba(184,160,106,.3)' }}>
  ...
  return (
    <div className="animate-fade-in delay-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          <h2 className="font-display text-2xl text-white" style={{ letterSpacing: '.02em' }}>
            PARTIDOS <span style={{ color: '#b8a06a' }}>({matches.length})</span>
          </h2>
          <div className="flex items-center gap-3">
            {liveCount > 0 && (
            ...
            <span className="font-body text-xs" style={{ color: '#a9adb1' }}>{scheduledCount} programados · {finishedCount} finalizados</span>
          </div>
        </div>
        ...
        <button
          ...
          style={{ background: 'rgba(184,160,106,.1)', border: '1px solid rgba(184,160,106,.3)', color: '#b8a06a' }}
          onMouseEnter={e => { if (!syncing) { e.currentTarget.style.background = '#b8a06a'; e.currentTarget.style.color = '#111214' } }}
          onMouseLeave={e => { e.currentTarget.style.background = 'rgba(184,160,106,.1)'; e.currentTarget.style.color = '#b8a06a' }}
        >
          {syncing
      ...
      <div
        ...
        style={{ background: '#fff', border: '1px solid #dedbd4' }}
      >
        {fasesDisponibles.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-body font-bold uppercase text-[10px] tracking-widest w-14 flex-shrink-0" style={{ color: '#a9adb1' }}>Fase</span>
            {['todas', ...fasesDisponibles].map(f => (
              <button key={f} type="button"
                ...
                style={{
                  fontSize: 11,
                  background: filtroFase === f ? '#b8a06a' : 'rgba(255,255,255,.06)',
                  border: `1px solid ${filtroFase === f ? '#b8a06a' : 'rgba(255,255,255,.12)'}`,
                  color: filtroFase === f ? '#111214' : 'rgba(255,255,255,.5)',
                }}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-body font-bold uppercase text-[10px] tracking-widest w-14 flex-shrink-0" style={{ color: '#a9adb1' }}>Estado</span>
          {['todos', 'programado', 'en_vivo', 'finalizado'].map(est => {
            ...
            return (
              <button key={est} type="button"
                ...
                style={{
                  fontSize: 11,
                  background: isActive ? (s?.bg || '#b8a06a') : 'rgba(255,255,255,.06)',
                  border: `1px solid ${isActive ? (s?.border || '#b8a06a') : 'rgba(255,255,255,.12)'}`,
                  color: isActive ? (s?.color || '#111214') : 'rgba(255,255,255,.5)',
                }}>
      ...
      {/* Contador */}
      <p className="text-xs font-body mb-3" style={{ color: '#a9adb1' }}>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl p-12 text-center"
          style={{ background: 'rgba(111,115,119,.08)', border: '1px dashed rgba(184,160,106,.2)' }}>
          <p className="font-body text-sm font-semibold" style={{ color: '#6f7377' }}>
          ...
          {filtered.map(match => {
            ...
            return (
              <div
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(184,160,106,.25)'; e.currentTarget.style.background = 'linear-gradient(155deg,rgba(111,115,119,.22),rgba(111,115,119,.08))' }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = isLive ? 'rgba(255,77,109,.3)' : 'rgba(255,255,255,.07)'; e.currentTarget.style.background = 'linear-gradient(155deg,rgba(111,115,119,.15),rgba(111,115,119,.04))' }}
              >
                <div className="flex items-center gap-3">
                  {/* Fase / estado */}
                  <div className="flex flex-col gap-0.5 flex-shrink-0 w-24 hidden sm:flex">
                    {faseLabel && (
                      <span className="text-[9px] font-semibold uppercase tracking-wider font-body" style={{ color: elim ? '#b8a06a' : '#a9adb1' }}>
                        {faseLabel}
                      </span>
                    )}
                    {match.grupo && (
                      <span className="text-[9px] font-body" style={{ color: '#a9adb1' }}>
                  ...
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 flex-1 min-w-0">
                    <div className="flex items-center gap-2 justify-end min-w-0">
                      {match.bandera_local && (
                        <img src={match.bandera_local} alt="" className="w-6 h-4 object-cover rounded-sm flex-shrink-0" style={{ border: '1px solid #d8d5ce' }} />
                      )}
                    ...
                    <div className="flex-shrink-0 px-3 py-1 rounded-lg text-center"
                      style={{ background: 'rgba(2,15,39,.5)', border: '1px solid #d8d5ce', minWidth: 56 }}>
                      {isScheduled ? (
                        <span className="font-display text-sm" style={{ color: '#a9adb1' }}>- : -</span>
                      ) : (
                        <>
                          <span className="font-display text-base" style={{ color: isLive ? '#ff4d6d' : '#b8a06a' }}>
                            {match.goles_local ?? 0} : {match.goles_visitante ?? 0}
                          </span>
                          {tienePenales && (
                            <span className="block text-[9px] font-body font-bold uppercase tracking-wider" style={{ color: '#6f7377' }}>
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {match.bandera_visitante && (
                        <img src={match.bandera_visitante} alt="" className="w-6 h-4 object-cover rounded-sm flex-shrink-0" style={{ border: '1px solid #d8d5ce' }} />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2">
                    ...
                    <button
                      ...
                      style={{ background: 'rgba(184,160,106,.1)', border: '1px solid #dedbd4', color: '#b8a06a' }}
                      onMouseEnter={e => { e.currentTarget.style.background = '#b8a06a'; e.currentTarget.style.color = '#111214' }}
                      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(184,160,106,.1)'; e.currentTarget.style.color = '#b8a06a' }}
