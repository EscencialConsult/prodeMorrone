    ...
    <div className="animate-fade-in delay-2">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          <h2 className="font-display text-2xl" style={{ color: '#202124', letterSpacing: '.02em' }}>PENDIENTES</h2>
      ...
      {loadingUsers && pendingUsers.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="font-body text-sm mt-3" style={{ color: '#6f7377' }}>Cargando usuarios...</p>
        </div>
      )}
      ...
      {!loadingUsers && pendingUsers.length === 0 && (
        <div className="rounded-2xl p-12 text-center" style={{ background: '#fff', border: '1px dashed #d8d5ce' }}>
          ...
          <p className="font-body font-semibold text-sm" style={{ color: '#6f7377' }}>No hay usuarios pendientes</p>
          <p className="font-body text-xs mt-1" style={{ color: '#a9adb1' }}>Las solicitudes nuevas van a aparecer acá.</p>
        </div>
      )}
          ...
          {pendingUsers.map(u => {
            ...
            return (
              <div key={u.id} className="rounded-2xl p-5 transition-all"
                style={{
                  background: '#fff',
                  border: `1px solid ${isApproving ? '#b8a06a' : '#dedbd4'}`,
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-display text-lg"
                      style={{ background: 'linear-gradient(135deg,#202124,#3b3d40)', color: '#b8a06a', boxShadow: '0 4px 12px rgba(32,33,36,.2)' }}>
                      {getInitials(u.nombre)}
                    </div>
                    <div className="min-w-0">
                      <p className="font-body font-semibold truncate" style={{ color: '#202124' }}>{u.nombre}</p>
                      <p className="text-xs font-body truncate" style={{ color: '#6f7377' }}>{u.email}</p>
                      <p className="font-body mt-0.5" style={{ fontSize: 10, color: '#a9adb1' }}>
                ...
                {/* Panel aprobación */}
                {isApproving && (
                  <div className="mt-5 pt-5 flex flex-col gap-5" style={{ borderTop: '1px solid #dedbd4' }}>

                    {/* Rol */}
                    <div>
                      <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>Rol</p>
                        ...
                        {[
                          ...
                          return (
                            <button key={opt.val} type="button"
                              ...
                              style={{
                                background: isActive ? '#202124' : '#f6f5f2',
                                border: `1px solid ${isActive ? '#202124' : '#d8d5ce'}`,
                                color: isActive ? '#b8a06a' : '#202124',
                              }}>
                              <p className="font-body font-semibold text-sm">{opt.label}</p>
                              <p className="font-body text-xs mt-0.5" style={{ color: isActive ? 'rgba(184,160,106,.6)' : '#6f7377' }}>{opt.desc}</p>
                            </button>
                          )
                        })}
                      </div>
                    ...
                    {/* Área */}
                    <div>
                      <p className="font-body font-bold text-xs uppercase tracking-widest mb-2" style={{ color: '#6f7377' }}>Área</p>
                      {areas.length === 0 ? (
                        <p className="text-xs font-body p-3 rounded-xl"
                          style={{ background: 'rgba(184,160,106,.06)', border: '1px solid rgba(184,160,106,.2)', color: '#8f7a45' }}>
