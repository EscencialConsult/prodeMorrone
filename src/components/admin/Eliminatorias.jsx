...
// âœ… Mundial 2026 tiene 48 equipos: Fase de Grupos â†’ 16avos (32 equipos) â†’ Octavos (16 equipos) â†’ etc.
...
/* â”€â”€ Sub-componentes de UI internos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function Field({ label, error, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="font-body font-bold text-xs uppercase tracking-widest"
        style={{ color: error ? '#e03252' : '#6f7377' }}>
        {label}
      </label>
      <input
        {...props}
        ...
        style={{ 
          background: '#fff', 
          border: `1px solid ${error ? '#e03252' : '#d8d5ce'}`, 
          color: '#202124' 
        }}
        onFocus={e => {
          e.target.style.borderColor = error ? '#e03252' : '#b8a06a'
          e.target.style.boxShadow = error 
            ? '0 0 0 3px rgba(224,50,82,.12)' 
            : '0 0 0 3px rgba(184,160,106,.12)'
        }}
        onBlur={e => {
          e.target.style.borderColor = error ? '#e03252' : '#d8d5ce'
...
function FilterChip({ active, onClick, children, disabled }) {
  return (
    <button
      ...
      style={{
        fontSize: 11,
        background: active ? '#202124' : '#fff',
        border: `1px solid ${active ? '#202124' : '#d8d5ce'}`,
        color: active ? '#b8a06a' : '#6f7377',
      }}
      onMouseEnter={e => { if (!active && !disabled) { e.currentTarget.style.borderColor = '#202124'; e.currentTarget.style.color = '#202124' } }}
      onMouseLeave={e => { if (!active && !disabled) { e.currentTarget.style.borderColor = '#d8d5ce'; e.currentTarget.style.color = '#6f7377' } }}
    >
      {children}
    </button>
  )
...
/* â”€â”€ Componente principal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
export default function CreateBetForm({ onSubmit, loading, matches = [] }) {
  ...
  async function handleSubmit(e) {
    ...
    if (form.partidos_ids.length === 0) {
      toast.error('Seleccioná al menos un partido para el pronóstico.')
      return
    }
    ...
    try {
      ...
      if (isPro && form.type === 'grupos') {
        if (form.areas_ids.length < 2) { 
          toast.error('Para pronósticos por áreas seleccioná al menos 2 áreas.')
          return 
        }
        payload.areas_ids = form.areas_ids.join(',')
      }
      await onSubmit(payload)
      toast.success('Pronóstico creado exitosamente')
      setForm(INITIAL)
      setFiltroFase('todas')
      setFiltroJornada('todas')
      setFiltroGrupo('todos')
      ...
      toast.error('Error al crear pronóstico: ' + err.message) 
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        ...
        {isPro && (
          <div className="flex flex-col gap-1.5">
            <span className="font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#6f7377' }}>
              Tipo
            </span>
      ...
      {/* Áreas â€” solo Plan Pro y tipo grupos */}
      {isPro && form.type === 'grupos' && (
        <div className="flex flex-col gap-2 p-3 rounded-xl"
          style={{ border: '1px solid rgba(184,160,106,.25)', background: 'rgba(184,160,106,.04)' }}>
          <span className="font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#6f7377' }}>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <span className="font-body text-xs" style={{ color: '#a9adb1' }}>
            <span className="font-bold" style={{ color: '#8f7a45' }}>{seleccionados}</span>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              <button
                ...
                style={{
                  background: filtroFase === 'todas' ? '#202124' : '#fff',
                  border: `1px solid ${filtroFase === 'todas' ? '#202124' : '#d8d5ce'}`,
                  color: filtroFase === 'todas' ? '#b8a06a' : '#6f7377',
                }}
              >
                Todas las fases
              </button>
              {fasesDisponibles.map(f => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: filtroFase === f ? '#202124' : '#fff',
                      border: `1px solid ${filtroFase === f ? '#202124' : '#d8d5ce'}`,
                      color: filtroFase === f ? '#b8a06a' : '#6f7377',
                    }}
                  >
                    {LABEL_FASE[f] || f}
                    {terminada && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {jornadasDisponibles.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-body font-semibold uppercase w-14"
                style={{ fontSize: 10, color: '#a9adb1', letterSpacing: '.1em' }}>Jornada</span>
          ...
          {gruposDisponibles.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-body font-semibold uppercase w-14"
                style={{ fontSize: 10, color: '#a9adb1', letterSpacing: '.1em' }}>Grupo</span>
        ...
        <input
          ...
          onFocus={e => { e.target.style.borderColor = '#b8a06a'; e.target.style.boxShadow = '0 0 0 3px rgba(184,160,106,.12)' }}
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors disabled:opacity-40" style={{ color: '#8f7a45' }}
          ...
          {seleccionados > 0 && (
            <button type="button" onClick={limpiarSeleccion}
              className="transition-colors" style={{ color: '#a9adb1' }}
              onMouseEnter={e => { e.currentTarget.style.color = '#e03252' }}
              onMouseLeave={e => { e.currentTarget.style.color = '#a9adb1' }}>
              Limpiar selección
            </button>
          )}
        </div>
        ...
        <div className="max-h-80 overflow-y-auto rounded-xl"
          style={{ background: '#f6f5f2', border: '1px solid #d8d5ce' }}>
          {agrupados.length === 0 ? (
            <p className="font-body text-xs text-center p-4" style={{ color: '#a9adb1' }}>
            ...
            return (
              <div key={`${gr.fase}-${gr.jornada}-${gr.grupo}`}>
                <div className="px-3 py-1.5 flex items-center justify-between sticky top-0 z-10"
                  style={{ background: '#202124', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
                  <span className="font-body font-semibold uppercase"
                    style={{ fontSize: 10, color: 'rgba(184,160,106,.8)', letterSpacing: '.1em' }}>
                    {header}
                  </span>
                ...
                {gr.partidos.map(m => {
                  ...
                  return (
                    <label key={m.id}
                      ...
                      style={{
                        background: checked ? 'rgba(184,160,106,.08)' : 'transparent',
                        borderBottom: '1px solid #dedbd4',
                      }}
                      onMouseEnter={e => { if (!checked) e.currentTarget.style.background = 'rgba(32,33,36,.04)' }}
                      onMouseLeave={e => { if (!checked) e.currentTarget.style.background = 'transparent' }}
                    >
                      <span className="flex-shrink-0 w-4 h-4 rounded flex items-center justify-center"
                        style={{
                          border: `1.5px solid ${checked ? '#b8a06a' : '#d8d5ce'}`,
                          background: checked ? '#b8a06a' : 'transparent',
                          transition: 'all .15s',
                        }}>
                        {checked && (
                          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#111214" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 font-body text-sm" style={{ color: '#202124' }}>
                          ...
                          <span style={{ color: '#a9adb1', fontSize: 11 }}>vs</span>
                      ...
                      <span className="font-body whitespace-nowrap" style={{ fontSize: 10, color: '#a9adb1' }}>
      ...
      <button
        ...
        style={{
          background: canSubmit ? '#202124' : 'rgba(32,33,36,.2)',
          color: canSubmit ? '#b8a06a' : '#a9adb1',
          boxShadow: canSubmit ? '0 4px 16px rgba(32,33,36,.2)' : 'none',
        }}
        onMouseEnter={e => { if (canSubmit) { e.currentTarget.style.background = '#3b3d40'; e.currentTarget.style.transform = 'translateY(-1px)' } }}
        onMouseLeave={e => { if (canSubmit) { e.currentTarget.style.background = '#202124'; e.currentTarget.style.transform = '' } }}
      >
        {loading
