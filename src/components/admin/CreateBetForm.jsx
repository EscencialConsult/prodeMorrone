  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        style={{ 
          background: '#fff', 
          border: `1.5px solid ${error ? '#e03252' : '#dedbd4'}`, 
          color: '#1f2023' 
        }}
        onFocus={e => {
          ...
          e.target.style.boxShadow = error 
            ...
            : '0 0 0 3px rgba(184,160,106,.08)'
        }}
        onBlur={e => {
          e.target.style.borderColor = error ? '#e03252' : '#dedbd4'
...
function FilterChip({ active, onClick, children, disabled }) {
  return (
    <button
      ...
      style={{
        background: active ? '#1f2023' : '#fff',
        border: `1.5px solid ${active ? '#1f2023' : '#dedbd4'}`,
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <span className="font-body text-xs" style={{ color: '#a9adb1' }}>
            <span className="font-bold" style={{ color: seleccionados > 0 ? '#8f7a45' : '#a9adb1' }}>
              {seleccionados}
            </span>
        ...
        <div className="flex flex-wrap gap-3">
          {jornadasDisponibles.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-body font-semibold uppercase text-[10px]"
                style={{ color: '#a9adb1', letterSpacing: '.1em', minWidth: 60 }}>Jornada</span>
          ...
          {gruposDisponibles.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="font-body font-semibold uppercase text-[10px]"
                style={{ color: '#a9adb1', letterSpacing: '.1em', minWidth: 50 }}>Grupo</span>
        ...
        <input
          ...
          className="w-full px-4 py-3 rounded-xl font-body text-sm outline-none transition-all"
          style={{ background: '#fff', border: '1.5px solid #dedbd4', color: '#1f2023' }}
          onFocus={e => { 
            e.target.style.borderColor = '#b8a06a'
            e.target.style.boxShadow = '0 0 0 3px rgba(184,160,106,.08)' 
          }}
          onBlur={e => { 
            e.target.style.borderColor = '#dedbd4'
            e.target.style.boxShadow = 'none' 
          }}
        />
        ...
        <div className="flex items-center justify-between gap-2 text-xs font-body">
          <button type="button" onClick={toggleVisibles} disabled={partidosFiltrados.length === 0}
            className="transition-colors disabled:opacity-40 font-semibold" style={{ color: '#8f7a45' }}
          ...
          {seleccionados > 0 && (
            <button type="button" onClick={limpiarSeleccion}
              className="transition-colors font-semibold" style={{ color: '#a9adb1' }}
        ...
        <div className="rounded-2xl overflow-hidden"
          style={{ border: '1.5px solid #dedbd4', boxShadow: '0 2px 8px rgba(32,33,36,0.04)' }}>
          <div className="max-h-96 overflow-y-auto"
            style={{ 
              background: '#fafafa',
              scrollbarWidth: 'thin',
              scrollbarColor: '#d8d5ce transparent'
            }}>
            {agrupados.length === 0 ? (
              <div className="text-center py-16"
                style={{ background: '#fff' }}>
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#d8d5ce" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="mx-auto mb-3">
                  <circle cx="11" cy="11" r="8"/>
                  <path d="m21 21-4.35-4.35"/>
                </svg>
                <p className="font-body text-sm" style={{ color: '#a9adb1' }}>
              ...
              return (
                <div key={`${gr.fase}-${gr.jornada}-${gr.grupo}`}>
                  ...
                  <div className="sticky top-0 z-10 px-4 py-2.5 flex items-center justify-between"
                    style={{ background: 'linear-gradient(135deg, #1f2023 0%, #2c2e31 100%)', borderBottom: '1px solid rgba(184,160,106,0.15)' }}>
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-md flex items-center justify-center font-display font-bold text-xs"
                        style={{ background: '#b8a06a', color: '#1f2023' }}>
                        {gr.partidos.length}
                      </span>
                  ...
                  {gr.partidos.map(m => {
                    ...
                    return (
                      <label
                        ...
                        style={{
                          background: checked ? 'rgba(184,160,106,0.08)' : '#fff',
                          borderBottom: '1px solid #f5f5f5',
                          opacity: yaTermino ? 0.5 : 1,
                          cursor: yaTermino ? 'not-allowed' : 'pointer',
                        }}
                        onMouseEnter={e => { 
                          if (!checked && !yaTermino) e.currentTarget.style.background = 'rgba(184,160,106,0.04)' 
                        }}
                        onMouseLeave={e => { 
                      ...
                      >
                        ...
                        <span className="flex-shrink-0 w-5 h-5 rounded flex items-center justify-center transition-all"
                          style={{
                            border: `2px solid ${checked ? '#b8a06a' : '#d8d5ce'}`,
                            background: checked ? '#b8a06a' : '#fff',
                          }}>
                          {checked && (
                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#1f2023" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {isPro && form.type === 'grupos' && (
        <div className="flex flex-col gap-3 p-4 rounded-xl"
          style={{ border: '1.5px solid rgba(184,160,106,.2)', background: 'rgba(184,160,106,.04)' }}>
          <span className="font-body font-bold text-xs uppercase tracking-widest" style={{ color: '#8f7a45' }}>
      ...
      <button
        ...
        style={{
          background: canSubmit ? 'linear-gradient(135deg, #1f2023 0%, #2c2e31 100%)' : 'rgba(32,33,36,.15)',
          color: canSubmit ? '#b8a06a' : '#a9adb1',
          boxShadow: canSubmit ? '0 4px 16px rgba(31,32,35,.2)' : 'none',
        }}
        onMouseEnter={e => { 
          if (canSubmit) { 
            e.currentTarget.style.background = 'linear-gradient(135deg, #2c2e31 0%, #1f2023 100%)'
            e.currentTarget.style.transform = 'translateY(-1px)'
            e.currentTarget.style.boxShadow = '0 6px 20px rgba(31,32,35,.25)'
          } 
        }}
        onMouseLeave={e => { 
          if (canSubmit) { 
            e.currentTarget.style.background = 'linear-gradient(135deg, #1f2023 0%, #2c2e31 100%)'
            e.currentTarget.style.transform = 'translateY(0)'
            e.currentTarget.style.boxShadow = '0 4px 16px rgba(31,32,35,.2)'
          } 
        }}
      >
        {loading ? (
