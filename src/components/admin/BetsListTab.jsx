...
function getBetStatus(bet) {
  ...
  if (bet.estado === 'finalizada')
    return { color: '#8f7a45', bg: 'rgba(184,160,106,.1)', border: 'rgba(184,160,106,.25)', label: 'Finalizada', dot: false }
  if (bet.estado === 'cerrada')
    return { color: '#6f7377', bg: 'rgba(95,110,138,.06)', border: 'rgba(95,110,138,.15)', label: 'Cerrada', dot: false }
  return { color: '#6f7377', bg: 'rgba(95,110,138,.06)', border: 'rgba(95,110,138,.12)', label: bet.estado || 'â€”', dot: false }
}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: '0 2px 8px rgba(32,33,36,.04)',
      }}
      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 20px rgba(32,33,36,.08)' }}
      onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '0 2px 8px rgba(32,33,36,.04)' }}
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span
              ...
              style={{ background: 'rgba(32,33,36,.04)', color: '#a9adb1', border: '1px solid #dedbd4' }}
            >
              {bet.tipo === 'grupos' ? 'Áreas' : 'Libre'}
            </span>
          </div>

          <p className="font-display text-lg truncate" style={{ color: '#202124', letterSpacing: '.01em' }}>
            {bet.titulo}
          </p>

          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            {matchCount > 0 && (
              <span className="flex items-center gap-1 text-xs font-body" style={{ color: '#6f7377' }}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                {matchCount} {matchCount === 1 ? 'partido' : 'partidos'}
              </span>
            )}
            {bet.premio && (
              <span className="flex items-center gap-1 text-xs font-body" style={{ color: '#8f7a45' }}>
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          ...
          {bet.estado === 'cerrada' && onFinalize && (
            <button
              ...
              style={{ fontSize: 11, background: 'rgba(184,160,106,.1)', border: '1px solid rgba(184,160,106,.35)', color: '#8f7a45' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#b8a06a'; e.currentTarget.style.color = '#111214' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'rgba(184,160,106,.1)'; e.currentTarget.style.color = '#8f7a45' }}
            >
              Finalizar
            </button>
          )}
...
export default function BetsListTab({ bets, loading, closeBet, finalizeBet }) {
  ...
  async function handleClose(id) {
    if (!window.confirm('¿Cerrar este pronóstico? Los usuarios ya no podrán cargar predicciones.')) return
    try { await closeBet(id) }
    catch (e) { alert('Error al cerrar: ' + e.message) }
  }

  async function handleFinalize(id) {
    if (!window.confirm('¿Finalizar este pronóstico? Esto calculará los puntajes finales.')) return
    ...
    <div className="flex flex-col gap-5 animate-fade-in delay-2">
      ...
      {loading && bets.length === 0 && (
        <div className="text-center py-16">
          <span className="inline-block w-8 h-8 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#b8a06a' }} />
          <p className="font-body text-sm mt-3" style={{ color: '#6f7377' }}>Cargando pronósticos...</p>
        </div>
      )}

      {!loading && bets.length === 0 && (
        <div className="rounded-2xl p-12 text-center" style={{ background: '#fff', border: '1px dashed #d8d5ce' }}>
          <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4"
            style={{ background: 'rgba(184,160,106,.08)', border: '1px solid rgba(184,160,106,.2)' }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#8f7a45" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <h3 className="font-display text-xl mb-2" style={{ color: '#202124', letterSpacing: '.02em' }}>
            No hay pronósticos creadas
          </h3>
          <p className="font-body text-sm" style={{ color: '#a9adb1' }}>
            Creá tu primera pronóstico desde la pestaña "Nuevo pronóstico"
          </p>
        </div>
      )}
      ...
      {openBets.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="font-display text-xl" style={{ color: '#202124', letterSpacing: '.02em' }}>ACTIVAS</h3>
      ...
      {closedBets.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="font-display text-xl" style={{ color: '#202124', letterSpacing: '.02em' }}>CERRADAS</h3>
            <span className="font-display text-lg" style={{ color: '#6f7377' }}>({closedBets.length})</span>
          </div>
          <div className="flex flex-col gap-3">
      ...
      {finishedBets.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="font-display text-xl" style={{ color: '#202124', letterSpacing: '.02em' }}>FINALIZADAS</h3>
            <span className="font-display text-lg" style={{ color: '#8f7a45' }}>({finishedBets.length})</span>
          </div>
          <div className="flex flex-col gap-3">
