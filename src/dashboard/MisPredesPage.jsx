...
const CARD={background:'#fff',border:'1px solid #dedbd4',borderRadius:14,boxShadow:'0 1px 0 rgba(32,33,36,.04)'}
const MUTED={fontSize:'.76rem',color:'#6f7377'}
...
// â”€â”€â”€ Helper: obtener nombre de equipo a partir de un código (AR / FRA / ...) â”€
...
// Por eso, traducir un código â†’ nombre solo requiere mirar al partido mismo.
...
function StatCard({label,value,color='#202124'}){
  return(
    <div style={{...CARD,padding:'1.1rem',textAlign:'center'}}>
      ...
      <p style={{fontSize:'.65rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'#a9adb1',margin:'.25rem 0 0'}}>{label}</p>
    </div>
  )
}
...
function PartidoRow({match,pred}){
  ...
  return(
    <div style={{padding:'.7rem .85rem',borderRadius:10,background:pts>0?'rgba(27,138,90,.04)':'rgba(32,33,36,.02)',border:`1px solid ${pts>0?'rgba(27,138,90,.2)':'#dedbd4'}`}}>
      {/* Equipos */}
      <div style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.5rem',marginBottom:'.45rem'}}>
        <div style={{display:'flex',alignItems:'center',gap:'.4rem',minWidth:0}}>
          {match.bandera_local&&<img src={match.bandera_local} alt="" style={{width:22,height:16,objectFit:'cover',borderRadius:2,border:'1px solid #dedbd4',flexShrink:0}}/>}
          <span style={{fontWeight:600,fontSize:'.82rem',color:'#202124',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{match.equipo_local}</span>
        </div>
        <span style={{fontSize:'.72rem',color:'#a9adb1'}}>vs</span>
        <div style={{display:'flex',alignItems:'center',gap:'.4rem',minWidth:0,justifyContent:'flex-end'}}>
          <span style={{fontWeight:600,fontSize:'.82rem',color:'#202124',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textAlign:'right'}}>{match.equipo_visitante}</span>
          {match.bandera_visitante&&<img src={match.bandera_visitante} alt="" style={{width:22,height:16,objectFit:'cover',borderRadius:2,border:'1px solid #dedbd4',flexShrink:0}}/>}
        </div>
      </div>

      {/* Scores */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'1.2rem',paddingTop:'.45rem',borderTop:'1px solid #ece9e2',flexWrap:'wrap'}}>
        {pred?(
          <div style={{textAlign:'center'}}>
            <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9adb1',margin:'0 0 2px'}}>{pred.es_grupal?'Área':'Tu predicción'}</p>
            <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:'#8f7a45',margin:0,lineHeight:1}}>{pred.pred_local} - {pred.pred_visitante}</p>
          </div>
        ):(
        ...
        {(fin||live)&&match.goles_local!=null&&(
          <>
            <div style={{width:1,height:28,background:'#dedbd4'}}/>
            <div style={{textAlign:'center'}}>
              <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9adb1',margin:'0 0 2px'}}>{live?'En vivo':'Resultado'}</p>
              <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:live?'#e03252':'#202124',margin:0,lineHeight:1}}>{match.goles_local} - {match.goles_visitante}</p>
              {tienePenales && (
                <p style={{fontSize:'.6rem',fontWeight:700,color:'#8f7a45',margin:'2px 0 0',textTransform:'uppercase',letterSpacing:'.06em'}}>
        ...
        {pts!=null&&pts!==''&&(
          <>
            <div style={{width:1,height:28,background:'#dedbd4'}}/>
            <div style={{textAlign:'center'}}>
              <p style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a9adb1',margin:'0 0 2px'}}>Puntos</p>
              <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'1.2rem',color:Number(pts)>0?'#1b8a5a':'#a9adb1',margin:0,lineHeight:1}}>+{pts}</p>
            </div>
          </>
        )}
      </div>

      {/* â˜… NUEVO: línea de "clasificado" para fases eliminatorias */}
      {esElim && (clasifPred || clasifRealCode) && (
        <div style={{marginTop:'.45rem',paddingTop:'.45rem',borderTop:'1px dashed #dedbd4',display:'flex',flexWrap:'wrap',gap:'.4rem .9rem',alignItems:'center',justifyContent:'center'}}>
          {clasifPred && (
            <span style={{display:'inline-flex',alignItems:'center',gap:'.3rem',fontSize:'.7rem',fontFamily:"'DM Sans',sans-serif"}}>
              <span style={{fontSize:'.58rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',color:'#a9adb1'}}>
                Tu clasif.:
              </span>
              <span style={{fontWeight:700, color: clasifRealCode ? (aciertoClasif ? '#1b8a5a' : '#e03252') : '#8f7a45'}}>
          ...
          {clasifRealCode && (
            <span style={{display:'inline-flex',alignItems:'center',gap:'.3rem',fontSize:'.7rem',fontFamily:"'DM Sans',sans-serif"}}>
              ...
              <span style={{fontWeight:700, color:'#202124'}}>{nombreDesdeCodigo(match, clasifRealCode)}</span>
            </span>
          )}
        </div>
      )}
    ...
    <AppShell>
      ...
      <div style={{maxWidth:1400,margin:'0 auto',padding:'2rem 1.5rem 3rem'}}>
        ...
        <div className="din" style={{marginBottom:'1.6rem'}}>
          <h1 style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:'clamp(2.4rem,6vw,3.5rem)',color:'#202124',margin:'0 0 .3rem',lineHeight:1,letterSpacing:'.02em'}}>MIS PREDICCIONES</h1>
        ...
        {(loading||lb)?(
          ...
          <div style={{...CARD,borderRadius:16,padding:'3rem',textAlign:'center'}}>
            <p style={{fontWeight:600,color:'#6f7377',margin:0}}>No hay pronósticos en esta categoría</p>
          </div>
        ):(
              ...
              return(
                <div key={bet.id} style={{...CARD,borderRadius:16,overflow:'hidden'}}>
                  ...
                  <div style={{padding:'1rem 1.2rem',display:'flex',alignItems:'center',gap:'1rem',cursor:'pointer'}}
                    ...
                    onMouseEnter={e=>{e.currentTarget.style.background='rgba(32,33,36,.02)'}}
                    onMouseLeave={e=>{e.currentTarget.style.background='transparent'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:'.55rem',marginBottom:'.22rem'}}>
                        <h3 style={{fontWeight:700,fontSize:'.92rem',color:'#202124',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{bet.titulo}</h3>
                  ...
                  {/* Expandido */}
                  {isExpanded&&(
                    <div style={{padding:'0 1.2rem 1.2rem',borderTop:'1px solid #ece9e2'}}>
