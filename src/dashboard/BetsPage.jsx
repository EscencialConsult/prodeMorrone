 ...
 *  - âœ… ARREGLADO: onFinalize cambiado a onClose para que el botón X funcione
 */
...
const STATE={
  ...
  cerrada:   {label:'CERRADA',  color:'#6f7377', bg:'rgba(95,110,138,.07)',border:'rgba(95,110,138,.18)'},
  finalizada:{label:'FINALIZADA',color:'#8f7a45',bg:'rgba(184,160,106,.08)',border:'rgba(184,160,106,.22)'},
}
...
function Toast({msg,ok}){
  if(!msg)return null
  return(
    <div style={{position:'fixed',top:'1.2rem',right:'1.2rem',zIndex:999,padding:'.75rem 1.1rem',borderRadius:12,background:ok?'#202124':'#fff',border:`1px solid ${ok?'rgba(184,160,106,.4)':'rgba(224,50,82,.3)'}`,boxShadow:'0 8px 28px rgba(32,33,36,.15)',display:'flex',alignItems:'center',gap:'.55rem',fontFamily:"'DM Sans',sans-serif",fontSize:'.84rem',fontWeight:600,color:ok?'#b8a06a':'#e03252',animation:'tin .3s ease both'}}>
      {ok
...
function BetCard({bet,predsMap,onPredict}){
  ...
  return(
    <div style={{...CARD_BASE,borderRadius:18,padding:'1.4rem 1.5rem',border:`1px solid ${live?'rgba(224,50,82,.25)':open?'rgba(27,138,90,.18)':'#dedbd4'}`,transition:'transform .2s,box-shadow .2s'}}
      onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 10px 28px rgba(32,33,36,.1)'}}
      onMouseLeave={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='0 1px 0 rgba(32,33,36,.04)'}}>
      ...
      {bet.partidos?.length>0&&(
        <div style={{display:'flex',flexDirection:'column',gap:'.4rem',marginBottom:'1.1rem'}}>
          {bet.partidos.slice(0,3).map(m=>{
            ...
            return(
              <div key={m.id} style={{display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center',gap:'.6rem',padding:'.55rem .75rem',borderRadius:9,background:pred?'rgba(184,160,106,.05)':'rgba(32,33,36,.02)',border:pred?'1px solid rgba(184,160,106,.2)':'1px solid #dedbd4'}}>
                <span style={{fontWeight:500,fontSize:'.8rem',color:'#202124',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.equipo_local}</span>
                ...
                <span style={{fontWeight:500,fontSize:'.8rem',color:'#202124',textAlign:'right',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{m.equipo_visitante}</span>
              </div>
            )
          })}
      ...
      {/* Footer */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem',paddingTop:'.85rem',borderTop:'1px solid #dedbd4'}}>
        ...
        {canPredict&&(
          <button onClick={()=>onPredict(bet)} style={{fontWeight:700,fontSize:'.8rem',padding:'.55rem 1.1rem',borderRadius:99,border:'none',background:'#b8a06a',color:'#111214',cursor:'pointer',transition:'all .17s',boxShadow:'0 4px 14px rgba(184,160,106,.25)',whiteSpace:'nowrap'}}
...
<p className="font-body text-sm" style={{ color: '#6f7377' }}>
  {filtered.length} {filtered.length === 1 ? 'pronóstico disponible' : 'pronósticos disponibles'}
            ...
            {FILTERS.map(f => {
              ...
              return (
                <button key={f.key} onClick={() => setFilter(f.key)}
                  ...
                  style={{
                    background: active ? '#202124' : 'transparent',
                    color: active ? '#b8a06a' : '#6f7377',
                    boxShadow: active ? '0 2px 8px rgba(32,33,36,.25)' : 'none',
                  }}
                  onMouseEnter={e => { if (!active) e.currentTarget.style.color = '#202124' }}
                  onMouseLeave={e => { if (!active) e.currentTarget.style.color = '#6f7377' }}
                >
                  {f.label}
                </button>
              )
        ...
        <div className="mb-6 animate-fade-in bg-white rounded-2xl border border-[#dedbd4] overflow-hidden">

          {/* Cabecera */}
          <button
            onClick={() => setShowPuntos(v => !v)}
            className="w-full flex items-center justify-between px-5 py-4 bg-transparent border-none cursor-pointer transition-all hover:bg-[rgba(32,33,36,.02)]"
            style={{ borderBottom: showPuntos ? '1px solid #dedbd4' : 'none' }}
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: 'linear-gradient(135deg,#202124,#6f7377)' }}>
              ...
              <div className="text-left">
                <p className="font-body font-bold text-sm text-[#202124] m-0">Sistema de puntos</p>
                <p className="font-body text-xs text-[#6f7377] m-0">Así se calcula tu puntaje en cada partido</p>
              </div>
            </div>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#a9adb1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
            ...
            <div className="px-5 pb-5 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  { pts: 5, titulo: 'Marcador exacto',  desc: 'Acertás el resultado final exacto del partido', accent: '#8f7a45', bg: 'rgba(184,160,106,.07)',  border: 'rgba(184,160,106,.28)' },
                  ...
                  <div key={pts} className="flex items-center gap-3 rounded-xl p-4"
                    ...
                    <div>
                      <p className="font-body font-bold text-sm m-0 mb-1" style={{ color: '#202124' }}>{titulo}</p>
                      <p className="font-body text-xs leading-snug m-0" style={{ color: '#6f7377' }}>{desc}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="rounded-xl p-4" style={{ background: 'rgba(244,180,42,.06)', border: '1px solid rgba(244,180,42,.25)' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={titulo} className="flex items-center gap-3 rounded-lg p-2.5" style={{ background: '#fff', border: '1px solid #dedbd4' }}>
                      <div className="flex flex-col items-center justify-center flex-shrink-0"
                        style={{ width: 38, height: 38, borderRadius: 8, background: 'rgba(244,180,42,.1)', border: '1px solid rgba(244,180,42,.3)' }}>
                        <span className="font-display leading-none" style={{ fontSize: '1.25rem', color: '#8f7a45' }}>{pts}</span>
                      </div>
                      <div className="min-w-0">
                        <p className="font-body font-bold text-xs m-0 mb-0.5" style={{ color: '#202124' }}>{titulo}</p>
                        <p className="font-body m-0" style={{ fontSize: '.7rem', color: '#6f7377', lineHeight: 1.3 }}>{desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <p className="font-body text-center mt-3" style={{ fontSize: '.68rem', color: '#a9adb1' }}>
        ...
        {loading?(
          <div style={{display:'grid',gap:'1rem'}}>
            {[1,2,3].map(i=><div key={i} style={{height:200,borderRadius:18,background:'#fff',border:'1px solid #dedbd4',animation:'skp 1.4s ease-in-out infinite'}}/>)}
          </div>
        ):filtered.length===0?(
          <div style={{borderRadius:18,padding:'3.5rem 1.5rem',textAlign:'center',background:'#fff',border:'1.5px dashed #dedbd4'}}>
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="#a9adb1" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{margin:'0 auto 1rem'}}><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>
            <p style={{fontWeight:600,fontSize:'.92rem',color:'#6f7377',margin:0}}>{filter==='activas'?'Todavía no hay pronósticos abiertos':filter==='cerradas'?'Todavía no hay pronósticos cerrados':'Todavía no hay pronósticos publicados'}</p>
            <p style={{fontSize:'.78rem',color:'#a9adb1',margin:'.4rem 0 0'}}>Te avisaremos cuando comience la próxima instancia del Prode MRE.</p>
          </div>
        ):(
      ...
      {/* âœ… Modal de predicción - ARREGLADO onClose */}
      {activeBet&&(
