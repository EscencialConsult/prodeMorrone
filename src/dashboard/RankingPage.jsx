/**
 * RankingPage.jsx â€” Diseño premium v6
...
/* Panel izquierdo */
.rk-sidebar { width:380px;flex-shrink:0;display:flex;flex-direction:column;overflow:hidden;background:#fbfaf7;border-right:1px solid #dedbd4 }
...
/* Fila apuesta */
.rk-row { display:flex;align-items:center;gap:10px;padding:11px 16px;cursor:pointer;border-bottom:1px solid #ece9e2;position:relative;transition:background .13s }
.rk-row:hover { background:rgba(32,33,36,.03) }
.rk-row.sel { background:#202124;border-bottom-color:rgba(255,255,255,.06) }
.rk-row::before { content:'';position:absolute;left:0;top:25%;bottom:25%;width:3px;background:#b8a06a;border-radius:0 3px 3px 0;opacity:0;transition:opacity .13s }
.rk-row.sel::before { opacity:1 }

/* Panel derecho */
.rk-content { flex:1;min-width:0;overflow-y:auto;background:#f6f5f2 }
...
.rk-trow:hover { background:rgba(32,33,36,.03) !important }

/* Skeleton */
.rk-sk { background:linear-gradient(90deg,rgba(32,33,36,.06) 25%,rgba(32,33,36,.1) 50%,rgba(32,33,36,.06) 75%);background-size:400px 100%;animation:rk-shimmer 1.4s ease infinite;border-radius:8px }
...
<div className="rk-in" style={{marginBottom:'1.25rem'}}>
        ...
        {/* Shell principal */}
<div className="rk-shell" style={{display:'flex',height:'calc(100vh - 162px)',minHeight:520,borderRadius:20,overflow:'hidden',boxShadow:'0 8px 48px rgba(32,33,36,.14)'}}>
...
function SideSection({ label, dot, children }) {
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:6,padding:'8px 16px 4px',position:'sticky',top:0,background:'#fbfaf7',zIndex:2}}>
...
function BetRow({ bet, sel, onPick }) {
  ...
  const col  = fin?'#b8a06a':open?'#22c55e':'#475569'
  ...
  return (
    <div className={`rk-row${sel?' sel':''}`} onClick={()=>onPick(bet)}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <p style={{fontSize:12,fontWeight:600,color:sel?'#fff':'#202124',margin:'0 0 2px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
          {bet.titulo}
        </p>
    ...
    <div style={{borderRadius:14,marginBottom:24,background:'linear-gradient(125deg,#202124 0%,#3b3d40 100%)',padding:'18px 22px',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:16,position:'relative'}}>
        <div>
          ...
          {apuesta.premio && (
            <div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(184,160,106,.1)',border:'1px solid rgba(184,160,106,.2)',borderRadius:99,padding:'3px 10px'}}>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#b8a06a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <span style={{fontSize:10,color:'rgba(184,160,106,.8)',fontWeight:600}}>{apuesta.premio}</span>
            </div>
          )}
        </div>
  ...
  return (
    <div style={{position:'relative',maxWidth:240,marginBottom:16}}>
      ...
      <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar participante..."
        style={{width:'100%',paddingLeft:28,paddingRight:q?28:10,paddingTop:7,paddingBottom:7,borderRadius:99,border:'1.5px solid #e2ddd6',background:'#fff',fontFamily:"'DM Sans',sans-serif",fontSize:12,color:'#202124',outline:'none',boxSizing:'border-box',transition:'border-color .15s'}}
        onFocus={e=>e.target.style.borderColor='rgba(184,160,106,.6)'}
        ...
        {orden.map(u => {
          ...
          return (
            <div key={u.user_id} className="rk-pcard"
              ...
              style={{
                background:'#fff',
                border:`${isTop||isExp?2:1.5}px solid ${isExp?'#b8a06a':isTop?cfg.border:'#e8e3db'}`,
                padding: isTop ? '20px 14px 14px' : '16px 12px 12px',
                boxShadow: isTop
                  ? `0 0 0 4px ${cfg.ring}, 0 12px 40px ${cfg.shadow}`
                  : isExp ? `0 0 0 3px rgba(184,160,106,.2)` : '0 2px 12px rgba(32,33,36,.06)',
              ...
              {/* CTA */}
              {puede && (
                <button style={{marginTop:10,background:'transparent',border:'none',cursor:'pointer',fontSize:10,fontWeight:700,color:isExp?'#8f7a45':'#94a3b8',display:'flex',alignItems:'center',justifyContent:'center',gap:3,width:'100%',padding:'4px 0',transition:'color .14s'}}
                  onMouseEnter={e=>e.currentTarget.style.color='#8f7a45'}
                  onMouseLeave={e=>e.currentTarget.style.color=isExp?'#8f7a45':'#94a3b8'}>
  ...
  return (
    <div style={{marginBottom:12}}>
      ...
      <div style={{borderRadius:14,overflow:'hidden',border:'1.5px solid #e8e3db',background:'#fff'}}>
        {/* Header */}
        <div style={{display:'grid',gridTemplateColumns:'44px 1fr 100px 68px 18px',padding:'8px 16px',background:'rgba(32,33,36,.02)',borderBottom:'1px solid #dedbd4',gap:8,alignItems:'center'}}>
        ...
        {filas.map((u,i)=>{
          ...
          return (
            <div key={u.user_id} style={{borderBottom:i<filas.length-1?'1px solid #ece9e2':'none'}}>
              <div className="rk-trow"
                onClick={puede?()=>onToggle(u.user_id):undefined}
                style={{
                  display:'grid',gridTemplateColumns:'44px 1fr 100px 68px 18px',
                  padding:'11px 16px',gap:8,alignItems:'center',
                  background:me?'rgba(184,160,106,.04)':isExp?'rgba(32,33,36,.015)':'#fff',
                  cursor:puede?'pointer':'default',
                }}>

                {/* Posición */}
                <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:pos<=3?'#b8a06a':'#c8c9cc',lineHeight:1}}>{pos}</span>

                {/* Participante */}
                <div style={{display:'flex',alignItems:'center',gap:10,minWidth:0}}>
                  <div style={{
                    width:34,height:34,borderRadius:'50%',flexShrink:0,
                    background:me?'linear-gradient(135deg,#b8a06a,#8f7a45)':'linear-gradient(135deg,#e2e8f0,#cbd5e1)',
                    display:'flex',alignItems:'center',justifyContent:'center',
                    fontFamily:"'Bebas Neue',sans-serif",fontSize:13,
                    color:me?'#202124':'#64748b',
                    boxShadow:me?'0 0 0 2px rgba(184,160,106,.3)':'none',
                  }}>{initials(u.nombre)}</div>
                  <div style={{minWidth:0}}>
                    <p style={{fontWeight:me?700:600,fontSize:13,color:me?'#8f7a45':'#202124',margin:'0 0 1px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                      {u.nombre}
                      {me&&<span style={{fontSize:10,color:'#94a3b8',fontWeight:400,marginLeft:4}}>(vos)</span>}
                    </p>
                ...
                <div className="rk-hide-mob" style={{display:'flex',justifyContent:'center',gap:8}}>
                  {[['#22c55e',u.aciertos_exactos,'Exactos'],['#b8a06a',u.aciertos_diferencia||0,'Dif.'],['#94a3b8',u.aciertos_resultado,'Res.']].map(([c,n,t])=>(
                ...
                {/* Puntos */}
                <div style={{textAlign:'center'}}>
                  <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:'#202124',lineHeight:1}}>{u.puntos_totales}</span>
                  <span style={{fontSize:8,color:'#94a3b8',marginLeft:2,letterSpacing:'.1em',fontWeight:700,display:'block'}}>PTS</span>
                </div>

                {/* Chevron */}
                {puede
                  ? <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={isExp?'#b8a06a':'#cbd5e1'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{transform:isExp?'rotate(180deg)':'none',transition:'transform .2s'}}>
                      <polyline points="6 9 12 15 18 9"/>
                    </svg>
                  : <span/>
                }
...
function DetallePanel({ nombre, det, apuesta, onClose, inline, style: extraStyle }) {
  const base = {
    ...
    overflow:'hidden',
    border: inline ? 'none' : '2px solid rgba(184,160,106,.4)',
    borderTop: inline ? '2px solid rgba(184,160,106,.25)' : undefined,
    boxShadow: inline ? 'none' : '0 4px 24px rgba(184,160,106,.1)',
  ...
  if (det.error) return (
    <div style={base}>
      <div style={{background:'#202124',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <span style={{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'.14em',color:'#f43f5e'}}>Error al cargar</span>
        <CloseBtn onClick={onClose}/>
      </div>
      <div style={{background:'#f6f5f2',padding:'12px 14px',fontSize:12,color:'#f43f5e'}}>{det.error}</div>
    </div>
  )
  ...
  return (
    <div style={base}>
      {/* Header */}
      <div style={{background:'linear-gradient(90deg,#202124,#3b3d40)',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{width:24,height:24,borderRadius:6,background:'rgba(184,160,106,.15)',border:'1px solid rgba(184,160,106,.25)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Bebas Neue',sans-serif",fontSize:10,color:'#b8a06a'}}>
            {initials(nombre)}
          </div>
          <span style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'rgba(184,160,106,.85)'}}>{nombre}</span>
        </div>
        <CloseBtn onClick={onClose}/>
      </div>

      {/* Body */}
      <div style={{background:'#f6f5f2',padding:'12px 14px 14px'}}>
        {/* Resumen chips */}
        {det.preds?.length>0 && (
          <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
            {[['exacto','#22c55e','Exactos'],['diferencia','#b8a06a','Diferencia'],['resultado','#94a3b8','Resultado'],['fallos','#f43f5e','Fallos']].map(([k,c,l])=>resumen[k]>0&&(
            ...
            {resumen.total>0 && (
              <span style={{marginLeft:'auto',fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:'#202124'}}>
    ...
    <div style={{
      ...
      <div>
        <div style={{display:'flex',alignItems:'center',gap:4,overflow:'hidden',marginBottom:2}}>
          ...
          <span style={{fontSize:11,fontWeight:600,color:'#202124',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{partido.equipo_local}</span>
          <span style={{fontSize:9,color:'#b8c0cc',flexShrink:0}}>vs</span>
          <span style={{fontSize:11,fontWeight:600,color:'#202124',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{partido.equipo_visitante}</span>
      ...
      <div style={{textAlign:'center'}}>
        ...
        {tieneScore
          ? <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:'#202124',margin:0,lineHeight:1}}>{partido.goles_local}-{partido.goles_visitante}</p>
    ...
    <div style={{position:'sticky',bottom:12,marginTop:12,zIndex:10}}>
      <div style={{borderRadius:13,overflow:'hidden',boxShadow:'0 8px 32px rgba(32,33,36,.28)',border:'2px solid rgba(184,160,106,.45)'}}>
        <div onClick={onToggle} style={{
          ...
          background:'linear-gradient(90deg,#202124,#3b3d40)',cursor:'pointer',
        }}>
          ...
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#b8a06a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
  ...
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8,fontSize:10,color:'#94a3b8',paddingTop:12,borderTop:'1px solid #e8e3db',marginTop:12}}>
      <div style={{display:'flex',flexWrap:'wrap',gap:10}}>
        {[['#22c55e',`Exacto +${e}pts`],['#b8a06a',`Diferencia +${d}pts`],['#94a3b8',`Resultado +${r}pt${r===1?'':'s'}`],['#f43f5e','Sin acierto 0pts']].map(([c,l])=>(
...
function EmptySelect() {
  return (
    <div style={{height:'100%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:40,textAlign:'center',gap:20}}>
      <div style={{width:80,height:80,borderRadius:24,background:'linear-gradient(145deg,#202124,#3b3d40)',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 12px 40px rgba(32,33,36,.2)'}}>
        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(184,160,106,.6)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>
        </svg>
      </div>
      <div>
        <p style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:'#202124',margin:'0 0 6px',letterSpacing:'.04em'}}>SELECCIONÁ UN PRONÓSTICO</p>
        <p style={{fontSize:13,color:'#94a3b8',margin:0,lineHeight:1.7}}>Elegí una instancia del panel de la<br/>izquierda para ver el ranking MRE.</p>
      </div>
      {/* Deco podio skeleton */}
      <div style={{display:'flex',gap:8,alignItems:'flex-end',opacity:.15,marginTop:8,pointerEvents:'none'}}>
        {[80,110,80].map((h,i)=>(
          <div key={i} style={{width:56,height:h,borderRadius:12,background:`linear-gradient(180deg,${i===1?'#b8a06a':'#cbd5e1'},transparent)`}}/>
        ))}
      </div>
    </div>
  )
...
function SinParticipantes() {
  return (
    <div style={{textAlign:'center',padding:'48px 24px',background:'#fff',borderRadius:14,border:'1.5px solid #e8e3db'}}>
      <p style={{fontWeight:700,color:'#64748b',margin:'0 0 6px',fontSize:15}}>Todavía no hay participantes</p>
      <p style={{fontSize:12,color:'#94a3b8',margin:0,lineHeight:1.6}}>Las predicciones aparecerán cuando los colaboradores las carguen.</p>
    </div>
  )
}
  ...
  return (
    <div style={{textAlign:'center',padding:32,background:'#fff',borderRadius:14,border:'1.5px solid #e8e3db',marginBottom:12}}>
      ...
      <button onClick={clear} style={{background:'transparent',border:'1.5px solid #e2ddd6',borderRadius:99,padding:'5px 16px',fontSize:12,fontWeight:600,color:'#64748b',cursor:'pointer',transition:'all .14s'}}
        onMouseEnter={e=>{e.currentTarget.style.borderColor='#202124';e.currentTarget.style.color='#202124'}}
