  /**
   * FixturePage.jsx â€” src/dashboard/FixturePage.jsx
   * 
   * CAMBIO: useBets() â†’ useFixtureSWR() 
  ...
  const ESTADO = {
    programado: { label:'Programado', color:'#6f7377', bg:'rgba(95,110,138,.07)', border:'rgba(95,110,138,.18)' },
    en_vivo:    { label:'EN VIVO',    color:'#e03252', bg:'rgba(224,50,82,.08)',  border:'rgba(224,50,82,.25)'  },
    finalizado: { label:'Finalizado', color:'#8f7a45', bg:'rgba(184,160,106,.08)', border:'rgba(184,160,106,.22)' },
    cancelado:  { label:'Cancelado',  color:'#a9adb1', bg:'rgba(168,178,196,.07)',border:'rgba(168,178,196,.2)' },
  }
  const FASES = {
  ...
  function Chip({ active, onClick, children }) {
    return (
      <button onClick={onClick} style={{ padding:'.32rem .75rem', borderRadius:99, fontFamily:"'DM Sans',sans-serif", fontWeight:700, fontSize:'.72rem', textTransform:'uppercase', letterSpacing:'.05em', cursor:'pointer', transition:'all .15s', background:active?'#202124':'#fff', color:active?'#b8a06a':'#6f7377', border:active?'1px solid transparent':'1px solid #dedbd4', boxShadow:active?'0 2px 8px rgba(32,33,36,.18)':'none' }}>
        {children}
      </button>
    )
  }
      ...
      <div
        ...
        style={{
          ...
          boxShadow: fin
            ...
            : '0 1px 0 rgba(32,33,36,.04)',
        }}
        onMouseEnter={e => {
          e.currentTarget.style.transform = 'translateY(-2px)'
          e.currentTarget.style.boxShadow = fin
            ? `0 1px 0 ${C.line}, 0 18px 40px -22px ${C.gold}55`
            : '0 8px 22px rgba(32,33,36,.09)'
        }}
        onMouseLeave={e => {
    ...
    if (!grupos.length) {
      return (
        ...
        >
          ...
          >
            <span className="text-xl" style={{ color: C.goldHi }}>ðŸ“Š</span>
          </div>
          <p
    ...
    return (
      <div
        ...
        style={{
          background: '#fff',
          border: `1px solid ${C.line}`,
          boxShadow: '0 1px 0 rgba(32,33,36,.04)',
        }}
        onMouseEnter={e => {
          e.currentTarget.style.transform = 'translateY(-2px)'
          e.currentTarget.style.boxShadow = '0 8px 24px rgba(32,33,36,.12)'
        }}
        onMouseLeave={e => {
          e.currentTarget.style.transform = 'none'
          e.currentTarget.style.boxShadow = '0 1px 0 rgba(32,33,36,.04)'
        }}
      >
        <div
          style={{
  ...
  export default function FixturePage() {
    // âœ… CAMBIO: useFixtureSWR() en lugar de useBets()
...
<div className="din" style={{ marginBottom:'1.5rem' }}>
  <h1 style={{
    ...
    <span style={{color:'#202124'}}>CRONOGRAMA </span>
    <span style={{color:'#b8a06a'}}>MUNDIALISTA</span>
  </h1>
  <p style={{ fontSize:'.84rem', color:'#6f7377', margin:0 }}>{matches?.length || 0} partidos del Mundial 2026</p>
          ...
          {loading && (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:'1rem' }}>
              {[...Array(6)].map((_,i)=><div key={i} style={{ height:100, borderRadius:14, background:'#fff', border:'1px solid #dedbd4', animation:'skp 1.4s ease-in-out infinite' }}/>)}
            </div>
          )}
          ...
          {!loading && tab==='fixture' && (
            <div className="din">
              ...
              {filtered.length===0
                ? <div style={{ borderRadius:18,padding:'3rem',textAlign:'center',background:'#fff',border:'1.5px dashed #dedbd4' }}>
                    <p style={{ fontWeight:600,color:'#6f7377',margin:0 }}>Todavía no hay partidos para mostrar</p>
                    <p style={{ fontSize:'.78rem',color:'#a9adb1',margin:'.4rem 0 0' }}>Cuando se actualice el fixture del Mundial, lo vas a ver acá.</p>
                  </div>
                : <div style={{ display:'flex', flexDirection:'column', gap:'2rem' }}>
                    {Object.entries(groups).map(([group,items])=>(
                      <div key={group}>
                        <p style={{ fontSize:'.7rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'rgba(184,160,106,.85)',marginBottom:'.7rem' }}>{group}</p>
